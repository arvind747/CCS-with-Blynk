...
extern void virtualWrite(uint8_t pin, uint16_t val);            //modified
...
void blynk_begin(uint8_t * auth, uint8_t * dest_ip, uint16_t dest_port, uint8_t * buf, uint8_t s)
{
	//s = socket; // modified for test

	signed char g_Host[] = BLYNK_DEFAULT_DOMAIN;
	unsigned int uiIP;
...
//    }
		...
		(BLYNK_SUCCESS != hdr.length && BLYNK_ALREADY_LOGGED_IN != hdr.length))
	{
		...
		else
		{
			...
			{
				//send(s, msgbuf, hsize);
				sl_Send(g_iSockID, msgbuf, hsize, 0);
			}
		}
		disconnect(g_iSockID);
...
void processInput(void)
{
	...
	switch (hdr.type)
	{
		...
		case BLYNK_CMD_BRIDGE:
		{
			...
			//printf("hdr.length = %d\r\n", hdr.length);
			if (hdr.length != sl_Recv(g_iSockID, msgbuf, hdr.length,0))
			{
...
void processCmd(uint8_t * buff, size_t len)
{
	...
	blynkparam_init(param2);                  //modified for test
...
//		rsp_len = sprintf((char *)rsp_mem, "dw %d %d ", pin, digitalRead(pin));
		rsp_len = sprintf((char *)rsp_mem, "dw %d %d ", pin, 12);
	...
	//	rsp_len = sprintf((char *)rsp_mem, "aw %d %d ", pin, analogRead(pin));
		rsp_len = sprintf((char *)rsp_mem, "aw %d %d ", pin, 2);
		replacetonull(rsp_mem, ' ');
		sendCmd(BLYNK_CMD_HARDWARE, 0, rsp_mem, rsp_len, NULL, 0);
	}
...
#endif
		rsp_len = sprintf((char *)rsp_mem, "vr %d %d ", pin, 90);
		replacetonull(rsp_mem, ' ');
		sendCmd(BLYNK_CMD_HARDWARE, 0, rsp_mem, rsp_len, NULL, 0);

		WidgetReadHandler handler;
		handler = GetReadHandler(1);
		if(handler)
		{
	...
	else
	{
		if(!strcmp(cmd, "vw")) // virtual pin write
		{
			...
			// update widget state
			rsp_len = sprintf((char *)rsp_mem, "vw %d %d ", pin, 40);
			replacetonull(rsp_mem, ' ');
			sendCmd(BLYNK_CMD_HARDWARE, 0, rsp_mem, rsp_len, NULL, 0);
...
#endif
		//if(BLINK_HEADER_SIZE != recv(s, msgbuf, BLINK_HEADER_SIZE))  // modified for test
	/*	if(BLINK_HEADER_SIZE != sl_Recv(g_iSockID, msgbuf, BLINK_HEADER_SIZE, 0))
		{
			return false;
		}
    */
	    sl_Recv(g_iSockID, msgbuf, BLINK_HEADER_SIZE, 0);    //modified
		...
		hdr->msg_id = sl_Htons(hdr->msg_id);
		hdr->length = sl_Htons(hdr->length);
#ifdef BLYNK_DEBUG
...
*/
    ...
    hdr.type = cmd;
    hdr.msg_id = sl_Htons(id);
    hdr.length = sl_Htons(length+length2);
    ...
    wlen += (size_t)sl_Send(g_iSockID, msgbuf, hsize, 0);
    ...
    if (cmd != BLYNK_CMD_RESPONSE) {
        if (length) {
           ...
           // wlen += (size_t)send(s, (uint8_t *)data, length);
			wlen += (size_t)sl_Send(g_iSockID, (uint8_t *)data, length, 0);
        }
        if (length2) {
#ifdef BLYNK_DEBUG
        	printf("<");
         ...
         //   wlen += (size_t)send(s, (uint8_t *)data2, length2);
			wlen += (size_t)sl_Send(g_iSockID, (uint8_t *)data2, length2, 0);
        }
